...
%%%% This code works with DREAM.3D version DREAM3D-v2.1681-Win64 
...
%vox_input = [1.5 1.5 1.5 1.5 1.5 3 3 3 3 3 6 6 6 6 6 10 10 10 10 10 30 30 30 30 30];
vox_input = [1.071428571 1.153846154 1.25 1.363636364 1.5 1.666666667 1.875 2.142857143 2.5 3 3.75 5 7.5 15];
tic % Clock file writing out speed

for vox_scalar = vox_input
%% ***** Define parameter values from input data *****

    % x,y,z edge lengths (microns)
    xbox_size = cube_dimension * xres_old;
    ybox_size = cube_dimension * yres_old;
    zbox_size = cube_dimension * zres_old;

    % down-sampled x,y,z resolutions (microns)
    xres_new = xres_old * vox_scalar;
    yres_new = yres_old * vox_scalar;
    zres_new = zres_old * vox_scalar;


    % Number of points along each dimension in the original structure
    xpoints = cube_dimension;
    ypoints = cube_dimension;
    zpoints = cube_dimension;

    % Number of points along each dimension in the down-sampled structure
...
%% ************ Write data to VTK ************
    ...
    seperator = ('_');
    ds_filename = strcat(num,seperator,random,seperator,filename);
...
%%%% ********** Write Header **********
    ...
    fprintf(fid, 'data set from software Version DREAM3D-v2.1681-Win64');
    fprintf(fid, '\n');
